    ...
    return (
        ...
        >
            <div className="h-[250px] sm:h-[200px] group relative bg-gray-100 rounded overflow-hidden flex items-center justify-center">
                ...
                <div className="absolute top-2 right-2 flex flex-col gap-2 z-10 items-end">
                    <div
                        ...
                        role="button"
                        tabIndex={0}
                        onKeyDown={(e) => (e.key === 'Enter' ? handleToggleWishList(e) : null)}
                    >
            ...
            <div className="py-2 px-2 sm:px-3">
                <h3 className="text-base sm:text-lg font-medium line-clamp-1 leading-snug">{productName}</h3>
                ...
                <div className="flex items-center gap-1 text-xs sm:text-sm pt-1">
                    {stars.map((type, index) =>
                        type === 'full' ? (
                            <StarIcon key={index} size={14} className="text-yellow-500 fill-yellow-500" />
                        ) : type === 'half' ? (
                            <StarHalf key={index} size={14} className="text-yellow-500 fill-yellow-500" />
                        ) : (
                            <StarOff key={index} size={14} className="text-gray-300 fill-none" />
                        )
                    )}
                    <span
                        className="text-gray-500 ml-1 cursor-pointer hover:underline text-xs sm:text-sm"
                        title="View Reviews"
                        onClick={(e) => {
                            e.stopPropagation();
                            openSidebar(reviews, rating);
                        }}
                    >
                        ({starCount})
                    </span>
